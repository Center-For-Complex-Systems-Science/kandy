...
ch_is_soz = np.zeros(n_ch, dtype=bool)  # True = actual SOZ cluster member
...
for ch in clusters_all[bulk_id]:
    ch_to_cl_idx[ch] = 0
    # ch_is_soz stays False for bulk

print(f"  Clusters: {[(c, len(clusters[c])) for c in KEEP]}")
print(f"  SOZ channels: {sum(ch_is_soz)}, Bulk: {sum(~ch_is_soz)}")
...
for bi, band_name in enumerate(BAND_NAMES):
    for ch in range(n_ch):
        ...
        if cl_mean_pre > 1e-6:
            ...
            ch_weight = 1.0
        # For bulk channels: keep pre-seizure amplitude, no onset spike/seizure ramp
        # They follow the phase dynamics but don't participate in the seizure burst
        if ch_is_soz[ch]:
            amp_base = amp_model[:, bi, ci] * ch_weight
        else:
            # Bulk: constant pre-seizure amplitude + slow weak recruitment after onset
            a_pre_val = amp_pre[band_name][ch]
            a_sez_val = amp_sez[band_name][ch]
            # Weak, delayed recruitment (20% of the SOZ amplitude change, 3s late)
            amp_base = np.array([
                sigmoid_ramp(t, a_pre_val,
                             a_pre_val + 0.2 * (a_sez_val - a_pre_val),
                             onset=ONSET_TIME + 3.0, tau=4.0)
                for t in t_sim
            ])
...
fig.tight_layout()
save_fig(fig, "cfc_v6_soz_channels")
...
fig.tight_layout()
save_fig(fig, "cfc_v6_full_array")
...
axes[-1, 1].set_xlabel("Time (s)")
fig.tight_layout()
save_fig(fig, "cfc_v6_seizure_zoom")
...
fig.tight_layout()
save_fig(fig, "cfc_v6_spectrogram")
...
fig.tight_layout()
save_fig(fig, "cfc_v6_pac_visualization")
