#!/usr/bin/env python3
"""Fully model-driven iEEG with CROSS-FREQUENCY COUPLING.

Same as reconstruct_model_only.py but adds phase-amplitude coupling (PAC):
  - Delta phase modulates beta/gamma amplitude
  - During seizure, the slow delta rhythm gates the fast activity
  - This produces the characteristic seizure pattern: slow rhythm with
    fast activity nested in the troughs

Cross-frequency coupling model:
  A_fast(t) = A_base(t) * (1 + kappa * cos(theta_delta(t) - phi_preferred))

  where:
    kappa = CFC strength (0 pre-seizure, increases at onset)
    phi_preferred = preferred delta phase for fast activity (trough = pi)
    A_base = sigmoid envelope (same as before)
...
The model generates:
  ...
  - Amplitudes: sigmoid ramp + delta-phase modulation (PAC)
...
def amplitude_model(t, band_name, cl_idx):
    """Model BASE amplitude at time t for a given band and cluster."""
...
# PAC: delta phase modulates fast band amplitudes
# kappa = coupling strength, ramps up at seizure onset
# phi_pref = preferred phase (pi = delta trough, where fast activity nests)
# CFC affects: alpha (weak), beta (moderate), low_gamma (strong)
CFC_KAPPA_PRE = 0.0     # no CFC pre-seizure
CFC_KAPPA_SEZ = {        # CFC strength at seizure, per fast band
    "delta":     0.0,     # delta doesn't modulate itself
    "theta":     0.1,     # weak
    "alpha":     0.3,     # moderate
    "beta":      0.6,     # strong
    "low_gamma": 0.8,     # strongest
}
CFC_PHI_PREF = np.pi    # fast activity nests at delta trough


def cfc_kappa(t, band_name):
    """CFC strength at time t for a given fast band."""
...
fig.tight_layout()
save_fig(fig, "cfc_pac_visualization")
