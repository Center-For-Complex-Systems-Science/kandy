#!/usr/bin/env python3
"""Fully model-driven iEEG v5: DC shift + pink noise.

v5 adds over v3:
  - DC/INFRASLOW SHIFT: Sub-1 Hz baseline shift at seizure onset. SOZ channels
    shift negative (matching real electrographic seizure pattern). Modeled as
    a slow sigmoid that adds a non-zero mean to the signal.
  - PINK (1/f) NOISE: Colored background noise per channel gives realistic
    spectral texture. Pre-seizure baseline is no longer pure sinusoids.

Seven seizure mechanisms:
  1. STAGGERED ONSET SPIKE: SOZ-first, then spreading outward
  2. CROSS-FREQUENCY COUPLING (PAC): delta phase gates fast-band amplitude
  3. DIRECTIONAL SYNCHRONIZATION: SOZ phase drives pacemaker/boundary
  4. AMPLITUDE FLUCTUATIONS: OU process on envelope (always present)
  5. Base Kuramoto frequency dynamics (from KANDy equations)
  6. DC/INFRASLOW SHIFT: sub-1 Hz baseline shift at seizure onset
  7. PINK NOISE: 1/f background on every channel

Data is used ONLY for static parameters. All dynamics from the model.

The model generates:
  - Phases: multiband Kuramoto + SOZ-driven phase attraction
  - Amplitudes: sigmoid ramp + onset spike + PAC + OU fluctuations
  - Baseline: DC shift + pink noise
...
# ---- STAGGERED ONSET TIMES (SOZ leads) ----
ONSET_DELAYS = {0: 0.0, 1: 0.5, 2: 1.5}  # C0 first, C2 +0.5s, C3 +1.5s


def cluster_onset(cl_idx):
    return ONSET_TIME + ONSET_DELAYS[cl_idx]


# ---- MECHANISM 1: ONSET SPIKE (staggered) ----
SPIKE_AMP = {0: 6.0, 1: 2.5, 2: 1.5}
SPIKE_WIDTH = 0.6


def onset_spike(t, cl_idx):
    """Multiplicative amplitude spike at cluster's staggered onset."""
    t_onset = cluster_onset(cl_idx)
    if t < t_onset - 2 * SPIKE_WIDTH:
        return 1.0
    dt = t - t_onset
    spike = SPIKE_AMP[cl_idx] * np.exp(-(dt / SPIKE_WIDTH) ** 2)
    return 1.0 + max(spike, 0.0)


# ---- MECHANISM 2: AMPLITUDE MODEL (base + spike) ----
def amplitude_base(t, band_name, cl_idx):
    """Base amplitude with staggered sigmoid transition."""
    a_pre = amp_pre[band_name][clusters[KEEP[cl_idx]]].mean()
    a_sez = amp_sez[band_name][clusters[KEEP[cl_idx]]].mean()
    t_onset = cluster_onset(cl_idx)
    return sigmoid_ramp(t, a_pre, a_sez, onset=t_onset, tau=RAMP_DUR)


def amplitude_model(t, band_name, cl_idx):
    """Base amplitude × onset spike."""
    return amplitude_base(t, band_name, cl_idx) * onset_spike(t, cl_idx)


# ---- MECHANISM 3: AMPLITUDE FLUCTUATIONS (Ornstein-Uhlenbeck) ----
# Pre-generate OU process for each band × cluster to give natural envelope
# breathing. This is always present (pre-seizure and seizure).
# dη = -η/τ_ou * dt + σ_ou * dW
OU_TAU = 2.0       # correlation time (seconds) — slow fluctuations
OU_SIGMA = 0.25    # amplitude of fluctuations (fraction of mean)

print("  Pre-generating amplitude fluctuations (OU process)...")
# We need these at simulation resolution
T_START_SIM = 60.0
T_END_SIM = 98.0
DT_SIM = 1.0 / FS
N_T_SIM = int((T_END_SIM - T_START_SIM) / DT_SIM)

ou_noise = {}  # ou_noise[(bi, ci)] = (N_T,) multiplicative noise
rng_ou = np.random.default_rng(SEED + 100)
for bi in range(N_BANDS):
    for ci in range(N_CL):
        eta = np.zeros(N_T_SIM)
        eta[0] = rng_ou.normal(0, OU_SIGMA)
        for i in range(1, N_T_SIM):
            eta[i] = eta[i-1] - eta[i-1] / OU_TAU * DT_SIM \
                     + OU_SIGMA * np.sqrt(2 * DT_SIM / OU_TAU) * rng_ou.normal()
        ou_noise[(bi, ci)] = eta


# ---- MECHANISM 6: DC / INFRASLOW SHIFT ----
# At seizure onset, SOZ channels develop a slow negative DC shift.
# This is sub-1 Hz activity not captured by our bandpass filters.
# Model: DC(t) = dc_amplitude * sigmoid(t - onset) per cluster
# SOZ shifts most, boundary less, bulk minimal
DC_AMP = {0: -80.0, 1: -30.0, 2: -15.0}  # amplitude per cluster (SOZ most negative)
DC_RAMP = 2.0  # seconds


def dc_shift(t, cl_idx):
    """DC baseline shift at seizure onset."""
    t_onset = cluster_onset(cl_idx)
    return sigmoid_ramp(t, 0.0, DC_AMP[cl_idx], onset=t_onset, tau=DC_RAMP)


# ---- MECHANISM 7: PINK (1/f) NOISE ----
# Generates colored noise with 1/f spectrum per channel.
# This gives the pre-seizure baseline its natural broadband texture.
print("  Pre-generating pink noise (1/f) per channel...")


def generate_pink_noise(n_samples, rng, alpha=1.0):
    """Generate 1/f^alpha noise via spectral method."""
    freqs = np.fft.rfftfreq(n_samples, d=DT_SIM)
    freqs[0] = 1.0  # avoid division by zero
    # 1/f^(alpha/2) amplitude spectrum
    amp_spectrum = 1.0 / freqs ** (alpha / 2.0)
    amp_spectrum[0] = 0.0  # no DC component in noise
    # Random phases
...
rng_pink = np.random.default_rng(SEED + 200)
# Pre-seizure std of the raw signal, per channel (for scaling pink noise)
# Use a fraction of the pre-seizure amplitude as noise level
PINK_NOISE_FRAC = 0.4  # pink noise is 40% of pre-seizure signal amplitude
pink_noise_all = np.zeros((N_T_SIM, n_ch))
for ch in range(n_ch):
    pn = generate_pink_noise(N_T_SIM, rng_pink, alpha=1.0)
    # Scale: pink noise amplitude proportional to channel's pre-seizure broadband amp
    ch_pre_amp = sum(amp_pre[bn][ch] for bn in BAND_NAMES)
    pn_scale = ch_pre_amp * PINK_NOISE_FRAC
    pink_noise_all[:, ch] = pn / np.std(pn) * pn_scale


# ---- MECHANISM 4: CROSS-FREQUENCY COUPLING (PAC) ----
CFC_KAPPA_PRE = 0.0
CFC_KAPPA_SEZ = {
    "delta":     0.0,
    "theta":     0.1,
...
def cfc_kappa(t, band_name, cl_idx):
    k_sez = CFC_KAPPA_SEZ[band_name]
    t_onset = cluster_onset(cl_idx)
    return sigmoid_ramp(t, CFC_KAPPA_PRE, k_sez, onset=t_onset)


# ---- MECHANISM 5: DIRECTIONAL SYNCHRONIZATION (SOZ drives others) ----
# SOZ (C0) phase drives pacemaker (C2) and boundary (C3)
# dθ_k/dt += K_sync(t) * sin(θ_SOZ - θ_k)  for k != SOZ
# The SOZ itself is NOT attracted to the others — it LEADS
SYNC_STRENGTH = 15.0
SYNC_DELAY = 1.0
SYNC_RAMP = 2.0


def sync_coupling(t, cl_idx):
    """Synchronization strength for cluster cl_idx.
    SOZ (idx 0) gets 0 (it leads). Others get ramped coupling."""
    if cl_idx == 0:
        return 0.0  # SOZ is the driver, not driven
    t_onset = cluster_onset(cl_idx)
    return sigmoid_ramp(t, 0.0, SYNC_STRENGTH,
                        onset=t_onset + SYNC_DELAY, tau=SYNC_RAMP)


# ---- KURAMOTO PHASE MODEL (with directional sync) ----
def multiband_rhs(t, theta):
    ...
    # DIRECTIONAL SYNC: SOZ drives pacemaker and boundary
    for bi in range(N_BANDS):
        s = bi * N_CL
        theta_soz = theta[s + 0]  # SOZ phase in this band
        for ci in range(1, N_CL):  # C2, C3 only (not SOZ)
            K_s = sync_coupling(t, ci)
            if K_s > 0.01:
                dtheta[s + ci] += K_s * np.sin(theta_soz - theta[s + ci])
...
# x_ch(t) = Σ_b A_base(t) * (1 + OU) * CFC_mod(t) * ch_weight * cos(θ(t) + δφ_ch)
X_model = np.zeros((N_T, n_ch))

# Precompute delta phases per cluster (for CFC modulation)
delta_phase_per_cl = theta_sim[:, 0:N_CL]  # delta band phases, shape (N_T, 3)
...
for bi, band_name in enumerate(BAND_NAMES):
    for ch in range(n_ch):
        ...
        # Base amplitude from model, scaled by channel weight
        ...
        if cl_mean_pre > 1e-6:
            ...
            ch_weight = 1.0
        amp_base = amp_model[:, bi, ci] * ch_weight

        # OU amplitude fluctuations (always present)
        ou = ou_noise[(bi, ci)][:N_T]
        amp_fluct = amp_base * (1.0 + ou)

        # Cross-frequency coupling: delta phase modulates fast-band amplitude
        kappa_t = np.array([cfc_kappa(t, band_name, ci) for t in t_sim])
        cfc_mod = 1.0 + kappa_t * np.cos(delta_phase_per_cl[:, ci] - CFC_PHI_PREF)

        amp = amp_fluct * cfc_mod
        X_model[:, ch] += amp * np.cos(phase)

print(f"  Oscillatory component: {X_model.shape}")

# Add DC/infraslow shift per channel
print("  Adding DC shift...")
for ch in range(n_ch):
    ci = ch_to_cl_idx[ch]
    dc = np.array([dc_shift(t, ci) for t in t_sim])
    # Scale DC by channel's relative amplitude (SOZ channels get more)
    cluster_chs = clusters[KEEP[ci]] if KEEP[ci] in clusters else clusters_all[bulk_id]
    cl_mean_pre = sum(amp_pre[bn][cluster_chs].mean() for bn in BAND_NAMES)
    ch_total_pre = sum(amp_pre[bn][ch] for bn in BAND_NAMES)
    ch_scale = ch_total_pre / max(cl_mean_pre, 1e-6)
    X_model[:, ch] += dc * ch_scale

# Add pink noise
print("  Adding pink noise...")
X_model += pink_noise_all[:N_T, :]
...
fig, axes = plt.subplots(len(soz_show), 2, figsize=(16, 2.5 * len(soz_show)),
                         sharex=True)
fig.suptitle("Multiband Kuramoto + Cross-Frequency Coupling\n"
             "Delta phase modulates beta/gamma amplitude at seizure onset",
             fontsize=14)
...
fig.tight_layout()
save_fig(fig, "cfc_v5_soz_channels")
...
fig.tight_layout()
save_fig(fig, "cfc_v5_full_array")
...
axes[-1, 1].set_xlabel("Time (s)")
fig.tight_layout()
save_fig(fig, "cfc_v5_seizure_zoom")
...
fig.tight_layout()
save_fig(fig, "cfc_v5_spectrogram")

# --- Plot 7: PAC visualization — delta phase vs fast amplitude ---
ch_pac = 25
fig, axes = plt.subplots(3, 1, figsize=(14, 10), sharex=True)
fig.suptitle(f"Cross-Frequency Coupling: Channel {ch_pac} (SOZ)\n"
             "Delta phase modulates beta/gamma amplitude at seizure onset",
             fontsize=14)

# Seizure window only
pac_mask = (t_sim >= ONSET_TIME - 2) & (t_sim <= ONSET_TIME + 8)
t_pac = t_sim[pac_mask]
ci_pac = ch_to_cl_idx[ch_pac]

# Top: delta oscillation
delta_phase = theta_sim[pac_mask, 0 * N_CL + ci_pac]
delta_sig = np.cos(delta_phase)
ax = axes[0]
ax.plot(t_pac, delta_sig, color="#1f77b4", lw=1)
ax.fill_between(t_pac, -1, delta_sig, where=delta_sig < 0,
                alpha=0.2, color="#1f77b4", label="Delta trough (fast activity nests here)")
ax.axvline(ONSET_TIME, color="red", ls="--", lw=1)
ax.set_ylabel(r"$\cos(\theta_\delta)$")
ax.set_title("Delta oscillation", fontsize=11)
ax.legend(fontsize=8)

# Middle: beta with CFC modulation
beta_phase = theta_sim[pac_mask, 3 * N_CL + ci_pac]
kappa_beta = np.array([cfc_kappa(t, "beta", ci_pac) for t in t_pac])
amp_beta_base = np.array([amplitude_model(t, "beta", ci_pac) for t in t_pac])
cfc_mod_beta = 1.0 + kappa_beta * np.cos(delta_phase - CFC_PHI_PREF)
beta_sig = amp_beta_base * cfc_mod_beta * np.cos(beta_phase)

ax = axes[1]
ax.plot(t_pac, beta_sig, color="#ff7f0e", lw=0.4)
ax.plot(t_pac, amp_beta_base * cfc_mod_beta, color="k", lw=1, alpha=0.5,
        label="Envelope (with CFC)")
ax.plot(t_pac, -amp_beta_base * cfc_mod_beta, color="k", lw=1, alpha=0.5)
ax.axvline(ONSET_TIME, color="red", ls="--", lw=1)
ax.set_ylabel("Beta signal")
ax.set_title("Beta (13-30 Hz) with delta-phase modulation", fontsize=11)
ax.legend(fontsize=8)

# Bottom: low gamma with CFC modulation
gamma_phase = theta_sim[pac_mask, 4 * N_CL + ci_pac]
kappa_gamma = np.array([cfc_kappa(t, "low_gamma", ci_pac) for t in t_pac])
amp_gamma_base = np.array([amplitude_model(t, "low_gamma", ci_pac) for t in t_pac])
cfc_mod_gamma = 1.0 + kappa_gamma * np.cos(delta_phase - CFC_PHI_PREF)
gamma_sig = amp_gamma_base * cfc_mod_gamma * np.cos(gamma_phase)

ax = axes[2]
ax.plot(t_pac, gamma_sig, color="#2ca02c", lw=0.3)
ax.plot(t_pac, amp_gamma_base * cfc_mod_gamma, color="k", lw=1, alpha=0.5,
        label="Envelope (with CFC)")
ax.plot(t_pac, -amp_gamma_base * cfc_mod_gamma, color="k", lw=1, alpha=0.5)
ax.axvline(ONSET_TIME, color="red", ls="--", lw=1)
ax.set_ylabel("Gamma signal")
ax.set_xlabel("Time (s)")
ax.set_title("Low gamma (30-50 Hz) with delta-phase modulation", fontsize=11)
ax.legend(fontsize=8)

fig.tight_layout()
save_fig(fig, "cfc_v5_pac_visualization")
...
print(f"  Amplitudes: sigmoid ramp + cross-frequency coupling (PAC)")
print(f"  CFC: delta phase modulates fast-band amplitude")
print(f"    kappa at seizure: theta={CFC_KAPPA_SEZ['theta']}, "
      f"alpha={CFC_KAPPA_SEZ['alpha']}, beta={CFC_KAPPA_SEZ['beta']}, "
      f"gamma={CFC_KAPPA_SEZ['low_gamma']}")
