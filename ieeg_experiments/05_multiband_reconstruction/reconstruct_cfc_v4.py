#!/usr/bin/env python3
"""Fully model-driven iEEG v3: SOZ-driven + amplitude fluctuations.

v3 fixes over v2:
  - SOZ-DRIVEN ONSET: Staggered onset times — C0 (SOZ) leads, C2 (pacemaker)
    follows ~0.5s later, C3 (boundary) ~1.5s later. Sync coupling is
    directional: SOZ drives the others.
  - AMPLITUDE FLUCTUATIONS: Ornstein-Uhlenbeck process on amplitude envelope
    gives natural breathing/fluctuations in pre-seizure baseline, matching
    the real iEEG's non-stationary envelope.

Five seizure mechanisms:
...
ch_is_bulk = np.zeros(n_ch, dtype=bool)  # track bulk channels
...
print(f"  Bulk channels: {sum(ch_is_bulk)} (will NOT receive onset spike)")
...
for bi, band_name in enumerate(BAND_NAMES):
    for ch in range(n_ch):
        ...
        # For bulk channels: use pre-seizure amplitude only (no spike, no seizure ramp)
        # They follow the SOZ phase but don't participate in the seizure amplitude
        if ch_is_bulk[ch]:
            amp_base_ch = np.array([amplitude_base(t, band_name, ci) for t in t_sim])
            # Bulk gets a much weaker seizure response
            a_pre_ch = amp_pre[band_name][ch]
            amp_base_ch = np.full(N_T, a_pre_ch)  # constant pre-seizure amplitude
            # Small gradual increase (bulk is recruited late and weakly)
            for ti in range(N_T):
                if t_sim[ti] > ONSET_TIME + 3.0:
                    frac = min((t_sim[ti] - ONSET_TIME - 3.0) / 5.0, 1.0)
                    amp_base_ch[ti] = a_pre_ch * (1.0 + 0.3 * frac)
            amp_base = amp_base_ch * ch_weight
        else:
            amp_base = amp_model[:, bi, ci] * ch_weight
        ...
        # Weaker for bulk channels
        kappa_scale = 0.2 if ch_is_bulk[ch] else 1.0
        kappa_t = np.array([cfc_kappa(t, band_name, ci) for t in t_sim]) * kappa_scale
...
fig.tight_layout()
save_fig(fig, "cfc_v4_soz_channels")
...
fig.tight_layout()
save_fig(fig, "cfc_v4_full_array")
...
axes[-1, 1].set_xlabel("Time (s)")
fig.tight_layout()
save_fig(fig, "cfc_v4_seizure_zoom")
...
fig.tight_layout()
save_fig(fig, "cfc_v4_spectrogram")
...
fig.tight_layout()
save_fig(fig, "cfc_v4_pac_visualization")
