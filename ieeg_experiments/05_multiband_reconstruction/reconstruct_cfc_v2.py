#!/usr/bin/env python3
"""Fully model-driven iEEG v2: CFC + Onset Spike + Post-Onset Synchronization.

Three seizure mechanisms beyond the base Kuramoto model:

  1. ONSET SPIKE: Brief (~1s) exponential amplitude burst at seizure onset,
     largest at SOZ, decaying with distance. Models the initial electrographic
     discharge.

  2. CROSS-FREQUENCY COUPLING (PAC): Delta phase modulates beta/gamma
     amplitude during seizure. Fast activity nests in delta troughs.

  3. POST-ONSET SYNCHRONIZATION: After the spike, a phase-attraction term
     pulls all clusters toward a common phase (per band). This models the
     sustained cross-channel synchronization observed in the data where
     all time series "go up together."

Data is used ONLY for static parameters (cluster assignments, phase offsets,
amplitude statistics). All dynamics from the model.

The model generates:
  - Phases: multiband Kuramoto + inter-cluster phase attraction
  - Amplitudes: sigmoid ramp + onset spike + delta-phase PAC
...
# ---- MECHANISM 1: ONSET SPIKE ----
# Brief exponential amplitude burst at seizure onset
# A_spike(t) = spike_amp * exp(-((t - onset) / spike_width)^2) for t > onset
# Largest at SOZ (cl_idx=0), weaker at pacemaker/boundary
SPIKE_AMP = {0: 5.0, 1: 2.0, 2: 1.5}   # multiplier per cluster (C0 strongest)
SPIKE_WIDTH = 0.8   # seconds (half-width of Gaussian pulse)


def onset_spike(t, cl_idx):
    """Multiplicative amplitude spike at onset. Returns multiplier >= 1."""
    if t < ONSET_TIME - 2 * SPIKE_WIDTH:
        return 1.0
    dt = t - ONSET_TIME
...
# ---- MECHANISM 2: AMPLITUDE MODEL (base) ----
def amplitude_model(t, band_name, cl_idx):
    ...
    a_base = sigmoid_ramp(t, a_pre, a_sez)
    return a_base * onset_spike(t, cl_idx)


# ---- MECHANISM 3: CROSS-FREQUENCY COUPLING (PAC) ----
CFC_KAPPA_PRE = 0.0
CFC_KAPPA_SEZ = {
    "delta":     0.0,
    "theta":     0.1,
...
def cfc_kappa(t, band_name):
    k_sez = CFC_KAPPA_SEZ[band_name]
    return sigmoid_ramp(t, CFC_KAPPA_PRE, k_sez)


# ---- MECHANISM 4: POST-ONSET SYNCHRONIZATION ----
# After the spike, clusters are attracted toward their band's mean phase.
# This is a standard Kuramoto coupling between clusters within each band:
#   dθ_k/dt += K_sync(t) * sin(θ_mean_band - θ_k)
# K_sync ramps up after onset (delayed relative to the spike)
SYNC_STRENGTH = 15.0    # coupling strength (rad/s) at full sync
SYNC_DELAY = 1.5        # seconds after onset before sync kicks in
SYNC_RAMP = 2.0         # ramp duration for sync


def sync_coupling(t):
    """Synchronization coupling strength. Ramps up after onset + delay."""
    return sigmoid_ramp(t, 0.0, SYNC_STRENGTH,
                        onset=ONSET_TIME + SYNC_DELAY, tau=SYNC_RAMP)


# ---- KURAMOTO PHASE MODEL (with synchronization) ----
def multiband_rhs(t, theta):
    r0 = r0_fn(t)
    r2 = r2_fn(t)
    r3 = r3_fn(t)
    K_sync = sync_coupling(t)
    ...
    # POST-ONSET SYNCHRONIZATION: pull all clusters toward band mean phase
    if K_sync > 0.01:
        for bi in range(N_BANDS):
            s = bi * N_CL
            band_phases = theta[s:s + N_CL]
            band_mean = np.angle(np.mean(np.exp(1j * band_phases)))
            for ci in range(N_CL):
                dtheta[s + ci] += K_sync * np.sin(band_mean - theta[s + ci])
...
# x_ch(t) = Σ_b A_base(t) * CFC_mod(t) * ch_weight * cos(θ(t) + δφ_ch)
# CFC_mod = 1 + kappa(t) * cos(θ_delta(t) - phi_pref)
...
for bi, band_name in enumerate(BAND_NAMES):
    for ch in range(n_ch):
        ...
        # A(t) = A_base(t) * (1 + kappa(t) * cos(theta_delta(t) - phi_pref))
        kappa_t = np.array([cfc_kappa(t, band_name) for t in t_sim])
        cfc_mod = 1.0 + kappa_t * np.cos(delta_phase_per_cl[:, ci] - CFC_PHI_PREF)

        amp = amp_base * cfc_mod
...
fig.tight_layout()
save_fig(fig, "cfc_v2_soz_channels")
...
fig.tight_layout()
save_fig(fig, "cfc_v2_full_array")
...
axes[-1, 1].set_xlabel("Time (s)")
fig.tight_layout()
save_fig(fig, "cfc_v2_seizure_zoom")
...
fig.tight_layout()
save_fig(fig, "cfc_v2_spectrogram")
...
kappa_beta = np.array([cfc_kappa(t, "beta") for t in t_pac])
...
kappa_gamma = np.array([cfc_kappa(t, "low_gamma") for t in t_pac])
...
fig.tight_layout()
save_fig(fig, "cfc_v2_pac_visualization")
