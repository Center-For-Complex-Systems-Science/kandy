...
Also uses direct matplotlib instead of plot_loss_curves() to avoid API issues.
...
X_dot   = (X_next - X_state) / dt                 # (7999, 3)  finite-diff deriv
...
try:
    ...
    print("  Diagnosis:")
    print("    • Device mismatch (fixed by CPU move in Step B)")
    print("    • Spline activations too complex for auto-symbolic fitting at 100 epochs")
...
if not kandy_kane_fitted:
    ...
    print("  If it persists, the KANELift lifts.py __call__ method needs")
    print("  an explicit .cpu() cast on the input tensor before forward().")
...
if kandy_kane_fitted:
    try:
        ...
        ax_a = axes2[0]
        ax_a.plot(true_traj_ref[:L, 0], true_traj_ref[:L, 2],
                  lw=0.4, alpha=0.7, color="steelblue", label="True")
        ax_a.plot(pred_traj_kane[:L, 0], pred_traj_kane[:L, 2],
                  lw=0.4, alpha=0.7, color="tomato", label="KANELift")
        ax_a.set_xlabel("x"); ax_a.set_ylabel("z")
        ax_a.set_title("Lorenz Attractor (x-z)")
