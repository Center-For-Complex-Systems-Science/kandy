...
Uses direct matplotlib instead of plot_loss_curves() to avoid API issues.
...
def move_lift_to_cpu(lift):
    ...
    converted to CPU tensors; if the encoder is on CUDA the spline grids
    (also on CUDA) clash with the CPU input tensor -- RuntimeError.
    """
    moved = []
...
# ──────────────────────────────────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP B: Training KANELift (latent_dim=8, hidden_dim=None)")
print("=" * 60)
...
if train_total:
    ...
    print("  No train loss recorded in history.")
if val_total:
    print(f"  Final val_total   loss : {val_total[-1]:.6f}")
else:
    print("  No val loss recorded in history.")
...
plot_data = {k: history[k] for k in plot_keys
...
fig.tight_layout()
fig.savefig("results/KANE/loss_curves.png", dpi=300, bbox_inches="tight")
plt.close(fig)
...
try:
    ...
    # 2. Forward pass -- encoder is on CPU, so use CPU tensor
    ...
    print("  Diagnosis:")
    print("    - Device mismatch (should be fixed by CPU move in Step B)")
    print("    - Spline activations too complex for auto-symbolic at 100 epochs")
    print("  Recommendation: run epochs>=200 and use auto_symbolic_with_costs()")
    print("    with POLY_LIB_CHEAP for better R2 coverage before extraction.")
...
if not kandy_kane_fitted:
    ...
    print("  Root cause: CUDA/CPU device mismatch in KANELift.__call__.")
    print("  The move_lift_to_cpu() helper above should resolve this.")
    print("  If it persists: patch lifts.py KANELift.__call__ to cast")
    print("  input tensor to the encoder device before forward pass.")

# ──────────────────────────────────────────────────────────────────────────
# STEP F: Rollout -- KANELift -- T=2000 steps
# ──────────────────────────────────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP F: Rollout and RMSE -- KANELift (T=2000, dt=0.01)")
print("=" * 60)
...
if kandy_kane_fitted:
    try:
        ...
        axes2[0].plot(true_traj_ref[:L, 0], true_traj_ref[:L, 2],
                      lw=0.4, alpha=0.7, color="steelblue", label="True")
        axes2[0].plot(pred_traj_kane[:L, 0], pred_traj_kane[:L, 2],
                      lw=0.4, alpha=0.7, color="tomato",    label="KANELift")
        axes2[0].set_xlabel("x"); axes2[0].set_ylabel("z")
        axes2[0].set_title("Lorenz Attractor (x-z): KANELift")
